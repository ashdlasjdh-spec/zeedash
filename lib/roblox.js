...
// fetch that retries Roblox 429 / 5xx with jittered backoff (Vercel's shared IPs get throttled).
async function robloxFetch(url, opts, tries = 5) {
  for (let a = 1; a <= tries; a++) {
    let res;
    try { res = await fetch(url, opts); } catch { if (a === tries) return null; await new Promise((r) => setTimeout(r, 300 * a)); continue; }
    if (res.ok || (res.status !== 429 && res.status < 500)) return res; // ok or non-retryable
    if (a === tries) return res;
    const ra = Number(res.headers.get("retry-after"));
    const wait = ra > 0 ? ra * 1000 + Math.random() * 300 : Math.min(6000, 400 * 2 ** (a - 1)) * (0.6 + Math.random() * 0.7);
