...
export function db() {
  if (!pool) {
    ...
    pool = new Pool({
      ...
      // CRITICAL on serverless: each Vercel function instance runs its own pool. pg defaults to
      // max:10, so under traffic dozens of instances × 10 connections exhaust Postgres'
      // max_connections and it starts refusing new ones ("too many clients") — which looks like the
      // DB "breaking after being up a while". Cap each instance to a couple of connections and let
      // the pool drain fully when idle so a frozen/torn-down lambda doesn't hold connections open.
  ...
  if (!schemaReady) {
    schemaReady = db()
      .query(`
        ...
        -- Tamper-evidence: each audit row hashes over the previous row's hash, forming a chain. Editing
        -- or deleting a past row breaks every hash after it, so silent DB edits are detectable.
...
export async function logAudit({ actorId, actorName, action, category = null, itemKey = null, target = null, detail = null }) {
  try {
    await ensureSchema();
    // Hash-chain: hash = sha256(prevHash + this row's fields). Chains every entry to the previous one.
    const prevRows = await query("select hash from audit_log order by id desc limit 1").catch(() => []);
    const prevHash = prevRows[0]?.hash || "";
    const ts = Date.now();
    const hash = crypto.createHash("sha256")
      .update(`${prevHash}|${actorId}|${action}|${category}|${itemKey}|${target}|${detail}|${ts}`)
      .digest("hex");
    await query(
      `insert into audit_log (actor_id, actor_name, action, category, item_key, target, detail, prev_hash, hash)
       values ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [actorId, actorName, action, category, itemKey, target, detail, prevHash || null, hash],
    );
  } catch (e) {
