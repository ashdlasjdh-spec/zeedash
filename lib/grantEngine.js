...
import { grantPerks, revokePerks, listPerks } from "@/lib/perksApi";
...
// Revoking a single power from an "All Powers" (_EveryPower) holder does nothing by default —
// _EveryPower supersedes individual powers, so there's no "Fly" entry to remove. This expands them
// to "every power EXCEPT the revoked one" (one whitelist read+write) so the revoke actually lands.
async function expandEveryPowerOnRevoke({ uid, key, by, byId, t }) {
  let row;
  try { const { perks } = await listPerks(); row = (perks || []).find((p) => String(p.userId) === String(uid)); }
  catch { return null; }
  const cur = Array.isArray(row?.powers) ? row.powers : [];
  if (!cur.includes("_EveryPower")) return null; // normal path handles ordinary revokes
  const all = (CATALOG.power || []).map((p) => p.key).filter((k) => k !== "_EveryPower");
  const keep = all.filter((k) => k !== key);
  try {
    const wl = (await dsGet("DashboardWhitelist", "powers")) || {};
    const id = Number(uid);
    const setP = (k, present) => {
      const list = Array.isArray(wl[k]) ? wl[k].map(Number) : [];
      wl[k] = present ? [...new Set([...list, id])] : list.filter((v) => v !== id);
    };
    setP("_EveryPower", false);
    setP(key, false);
    for (const k of keep) setP(k, true);
    await dsSet("DashboardWhitelist", "powers", wl);
  } catch { /* whitelist write failed — DB sync below still runs */ }
  try { await publish(t.powerRemove, { UserId: uid, Power: key, Admin: by }); } catch {}
  try { await publish("DashboardGrant", { action: "revoke", userId: uid, category: "power", key, by: byId }); } catch {}
  let warn = null;
  try {
    await revokePerks(uid, "power:_EveryPower", by);
    if (keep.length) await grantPerks(uid, { powers: keep }, by);
  } catch (e) { warn = `Expanded All Powers in-game, but DB sync failed: ${e.message}`; }
  return { warn };
}
...
export async function applyGrant({ category, key, uid, by, byId, action = "grant" }) {
  ...
  // Special case: revoking one power from an All-Powers holder → expand to all-but-that-one.
  if (category === "power" && revoke && key !== "_EveryPower") {
    const expanded = await expandEveryPowerOnRevoke({ uid, key, by, byId, t });
