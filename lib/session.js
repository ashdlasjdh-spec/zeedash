...
import { labelForLevel, canGroupScoped, MANUAL_PERMS } from "./permissions";
...
async function liveGuildGrants(discordId, serverPerms) {
  ...
  if (c && Date.now() - c.at < LEVEL_TTL_MS) return c.v;
  let v = c ? c.v : { manualPerms: {}, securityGuildIds: [] };
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, securityGuildIds: [] }; grantsCache.set(discordId, { v, at: Date.now() }); return v; }
    ...
    v = { manualPerms, securityGuildIds };
...
export async function getSession() {
  ...
  const serverPerms = await liveServerPerms(payload.id); // { guildId: {a,o} } — Discord admin/owner
  const { manualPerms, securityGuildIds } = await liveGuildGrants(payload.id, serverPerms); // fake-perms + antinuke-admin
  // A guild is reachable in the Server section via ANY standing: Discord admin/owner, a manual
  // permission held there, or being a listed antinuke admin.
  const adminGuildIds = Object.keys(serverPerms).filter((g) => serverPerms[g]?.a || serverPerms[g]?.o);
  const serverGuildIds = [...new Set([...adminGuildIds, ...Object.keys(manualPerms), ...securityGuildIds])];
  ...
  return { ...payload, level: eff, role: labelForLevel(eff), scopedGroup, scope, serverPerms, manualPerms, securityGuildIds, serverGuildIds, gameAccess };
}
