...
async function liveGuildGrants(discordId, serverPerms) {
  ...
  let v = c ? c.v : { manualPerms: {}, featureGrants: {}, securityGuildIds: [], sharedGuildIds: [], group: null, transcriptGuilds: [] };
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, featureGrants: {}, securityGuildIds: [], sharedGuildIds: [], group: null, transcriptGuilds: [] }; const at = Date.now(); grantsCache.set(discordId, { v, at }); kvSetJSON(`sess:grants:${discordId}`, { v, at }, LEVEL_TTL_MS / 1000); return v; }
    ...
    v = { manualPerms, featureGrants, securityGuildIds, sharedGuildIds: shared, group, transcriptGuilds: [...transcriptGuilds] };
    const at = Date.now();
...
export const getSession = cache(async () => {
  ...
  const [scope, { manualPerms, featureGrants, securityGuildIds, sharedGuildIds, group, transcriptGuilds }] = await Promise.all([
  ...
  return { ...payload, level: eff, role: labelForLevel(eff), isOwner: owner, scopedGroup, scope, serverPerms, manualPerms, featureGrants: featureGrants || {}, securityGuildIds, serverGuildIds, group: roleGroup, transcriptGuilds: transcriptGuilds || [], gameAccess };
});
