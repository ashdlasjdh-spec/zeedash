...
export const getSession = cache(async () => {
  ...
  if (blacklisted) return null; // blocked from the dashboard
  ...
  const [scope, { manualPerms, securityGuildIds }] = await Promise.all([
    liveScope(payload.id, level),              // string[] of scope keys
    liveGuildGrants(payload.id, serverPerms),  // fake-perms + antinuke-admin
  ]);
