...
export async function getSession() {
  ...
  const eff = level > 0 ? level : 1; // scoped-only users (e.g. Star Overseer) get base level to navigate
  return { ...payload, level: eff, role: labelForLevel(eff), scopedGroup, scope };
}
