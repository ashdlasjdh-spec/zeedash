...
async function liveGuildGrants(discordId, serverPerms) {
  ...
  let v = c ? c.v : { manualPerms: {}, securityGuildIds: [], sharedGuildIds: [], group: null };
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, securityGuildIds: [], sharedGuildIds: [], group: null }; const at = Date.now(); grantsCache.set(discordId, { v, at }); kvSetJSON(`sess:grants:${discordId}`, { v, at }, LEVEL_TTL_MS / 1000); return v; }
    ...
    v = { manualPerms, securityGuildIds, sharedGuildIds: shared, group };
    const at = Date.now();
...
export const getSession = cache(async () => {
  ...
  const [scope, { manualPerms, securityGuildIds, sharedGuildIds, group }] = await Promise.all([
  ...
  return { ...payload, level: eff, role: labelForLevel(eff), isOwner: owner, scopedGroup, scope, serverPerms, manualPerms, securityGuildIds, serverGuildIds, group: roleGroup, gameAccess };
});
