...
async function liveGuildGrants(discordId, serverPerms) {
  ...
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, securityGuildIds: [], sharedGuildIds: [] }; const at = Date.now(); grantsCache.set(discordId, { v, at }); kvSetJSON(`sess:grants:${discordId}`, { v, at }, LEVEL_TTL_MS / 1000); return v; }
