...
import { labelForLevel, canGroupScoped, MANUAL_PERMS, SITE_CAPS, isSuperOwner } from "./permissions";
...
async function liveGuildGrants(discordId, serverPerms) {
  ...
  let v = c ? c.v : { manualPerms: {}, securityGuildIds: [], sharedGuildIds: [], caps: [] };
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, securityGuildIds: [], sharedGuildIds: [], caps: [] }; const at = Date.now(); grantsCache.set(discordId, { v, at }); kvSetJSON(`sess:grants:${discordId}`, { v, at }, LEVEL_TTL_MS / 1000); return v; }
    ...
    // Role Access → site capabilities (crew tags, grants, bans, …). Applies to anyone holding a mapped
    // role, regardless of Discord admin status (it grants GAME abilities, not server-feature access).
    for (const [gid, cfg] of raByGuild) {
      const roleIds = rolesByGuild.get(gid) || [];
      if (!roleIds.length) continue;
      for (const it of cfg.items || []) {
        if (it && roleIds.includes(String(it.role))) {
          for (const cap of (Array.isArray(it.caps) ? it.caps : [])) if (SITE_CAPS.includes(cap)) capsSet.add(cap);
        }
      }
    }
    v = { manualPerms, securityGuildIds, sharedGuildIds: shared, caps: [...capsSet] };
    const at = Date.now();
...
export const getSession = cache(async () => {
  ...
  const [scope, { manualPerms, securityGuildIds, sharedGuildIds, caps }] = await Promise.all([
    liveScope(payload.id, level),              // string[] of scope keys
    liveGuildGrants(payload.id, serverPerms),  // fake-perms + antinuke-admin + role-access caps + shared guilds
  ]);
  const scopedGroup = scope.length > 0;
  const siteCaps = Array.isArray(caps) ? caps : [];
  ...
  // Game/whitelist section: Roblox staff ladder, scoped group, OR a Role-Access capability.
  const gameAccess = owner || level > 0 || scopedGroup || siteCaps.length > 0;
  ...
  // Game capability unlocks; scoped-only users get base 1. Role-Access caps are additive on top.
  const eff = owner ? 255 : (level > 0 ? level : (scopedGroup ? 1 : 0));
  return { ...payload, level: eff, role: labelForLevel(eff), isOwner: owner, scopedGroup, scope, serverPerms, manualPerms, securityGuildIds, serverGuildIds, caps: siteCaps, gameAccess };
});
