...
import { labelForLevel, canGroupScoped, MANUAL_PERMS, GROUP_ACTIONS, SECTION_GRANTS, isSuperOwner } from "./permissions";
import { NONSECURITY_FEATURES } from "./serverFeatures";
import { isExpired } from "./fakePerms";
...
// Login cookie lifetime. Kept short (3 days) so a leaked token expires on its own; the middleware
// slides it forward on every active page load, so real staff are never signed out mid-use. Must match
// SESSION_TTL_S in middleware.js.
const SESSION_TTL_S = 60 * 60 * 24 * 3;
export async function createSession(user) {
  ...
  (await cookies()).set(COOKIE, token, { httpOnly: true, secure: true, sameSite: "lax", path: "/", maxAge: SESSION_TTL_S });
}
export async function clearSession() { (await cookies()).set(COOKIE, "", { httpOnly: true, path: "/", maxAge: 0 }); }

// Global "sign everyone out" kill-switch. revokeAllSessions() records a cutoff; any token issued before
// it is then rejected at getSession(), forcing a fresh Discord login. Off by default (no key → cutoff 0
// → no effect), and the read is cached in-process for 60s + fails OPEN, so it can never wrongly lock
// people out from a KV hiccup. Use it if a token is ever believed compromised.
const KILL_KEY = "sess:killBefore";
let killCache = { v: 0, at: 0 };
async function killBefore() {
  if (Date.now() - killCache.at < 60000) return killCache.v;
  try { const v = await kvGetJSON(KILL_KEY); killCache = { v: Number(v) || 0, at: Date.now() }; }
  catch { killCache = { v: killCache.v, at: Date.now() }; } // fail open — keep last known
  return killCache.v;
}
export async function revokeAllSessions() {
  const cutoff = Math.floor(Date.now() / 1000);
  try { await kvSetJSON(KILL_KEY, cutoff, 30 * 24 * 3600); } catch { /* no KV — nothing to revoke against */ }
  killCache = { v: cutoff, at: Date.now() };
}
...
const grantsCache = new Map(); // id -> { v, at, ver }
// A global "grants version": bumped whenever Role Access / Fake Permissions change, so every user's
// cached grants are treated as stale on their next request — access changes apply immediately instead
// of waiting out the 90s TTL. When there's no KV (ver == null) each serverless call resolves fresh.
const GRANTS_VER_KEY = "grants:ver";
async function currentGrantsVer() { try { return await kvGetJSON(GRANTS_VER_KEY); } catch { return null; } }
export async function bumpGrantsVersion() {
  grantsCache.clear();
  try { await kvSetJSON(GRANTS_VER_KEY, Date.now(), 30 * 24 * 3600); } catch { /* no KV — fine */ }
}
export async function liveGuildGrants(discordId, serverPerms) {
  const ver = await currentGrantsVer(); // null when KV isn't configured
  const fresh = (rec) => rec && Date.now() - rec.at < LEVEL_TTL_MS && (ver == null || rec.ver === ver);
  const c = grantsCache.get(discordId);
  if (fresh(c)) return c.v;
  // Try the SHARED Redis cache before the DB + per-guild Discord lookups.
  const cached = await kvGetJSON(`sess:grants:${discordId}`);
  if (cached && cached.v && typeof cached.v === "object" && fresh(cached)) {
    grantsCache.set(discordId, { v: cached.v, at: cached.at, ver: cached.ver });
    return cached.v;
  }
  let v = c ? c.v : { manualPerms: {}, featureGrants: {}, securityGuildIds: [], sharedGuildIds: [], group: null, transcriptGuilds: [], sectionGrants: [] };
  try {
    ...
    if (!shared.length) { v = { manualPerms: {}, featureGrants: {}, securityGuildIds: [], sharedGuildIds: [], group: null, transcriptGuilds: [], sectionGrants: [] }; const at = Date.now(); grantsCache.set(discordId, { v, at, ver }); kvSetJSON(`sess:grants:${discordId}`, { v, at, ver }, LEVEL_TTL_MS / 1000); return v; }
    ...
    const manualPerms = {};
    const featureGrants = {}; // { guildId: { slug: { access:"view"|"manage", channels:[...] } } } — direct grants
    ...
    for (const [gid, cfg] of fpByGuild) {
      ...
      const held = new Set();
      const grants = {}; // slug -> { access, channels } (union across the member's mapped roles)
      ...
      for (const it of cfg.items || []) {
        if (it && roleIds.includes(String(it.role)) && !isExpired(it)) {
          for (const perm of String(it.perms || "").split(/[\s,]+/).filter(Boolean)) if (MANUAL_PERMS.has(perm)) held.add(perm);
          // Direct per-feature grants. Accepts the new array-of-objects shape { slug, access, channels }
          // or a legacy comma-string (treated as full "manage"). Never security / fake-permissions.
          const raw = it.features;
          const list = Array.isArray(raw)
            ? raw
            : String(raw || "").split(/[\s,]+/).filter(Boolean).map((s) => ({ slug: s, access: "manage" }));
          for (const f of list) {
            const slug = String(f?.slug ?? f ?? "").trim();
            if (!GRANTABLE_FEATURES.has(slug)) continue;
            const access = f?.access === "view" ? "view" : "manage";
            const channels = Array.isArray(f?.channels)
              ? f.channels.map(String)
              : String(f?.channels || "").split(/[\s,]+/).filter(Boolean);
            const prev = grants[slug];
            if (!prev) grants[slug] = { access, channels };
            else grants[slug] = {
              // manage wins over view; an empty channel list means "all channels", so it wins too.
              access: prev.access === "manage" || access === "manage" ? "manage" : "view",
              channels: prev.channels.length === 0 || channels.length === 0 ? [] : [...new Set([...prev.channels, ...channels])],
            };
          }
        }
      }
      // A Discord admin already manages every non-security feature, so their manual perms are moot —
      // but keep the (harmless, additive) direct feature grants.
      if (!isAdminHere && held.size) manualPerms[gid] = [...held];
      if (Object.keys(grants).length) featureGrants[gid] = grants;
    }
    ...
    const transcriptGuilds = new Set(); // guilds where a mapped role may view ticket transcripts
    const sectionGrants = new Set();    // delegated dashboard sections: bans / powers / grants
    for (const [gid, cfg] of raByGuild) {
      const roleIds = rolesByGuild.get(gid) || [];
      for (const it of cfg.items || []) {
        if (!it || isExpired(it)) continue;
        // An item applies if the member holds its role, OR it targets this user by id directly (so a
        // user-id grant works even when they don't have the role).
        const byRole = it.role && roleIds.includes(String(it.role));
        const byUser = it.user && String(it.user) === String(discordId);
        ...
        if (it.transcripts) transcriptGuilds.add(String(gid));
        for (const s of (Array.isArray(it.sections) ? it.sections : [])) if (SECTION_GRANTS.includes(s)) sectionGrants.add(s);
      }
    }
    const group = groupActions.size ? { actions: [...groupActions], maxRank: groupMaxRank } : null;
    v = { manualPerms, featureGrants, securityGuildIds, sharedGuildIds: shared, group, transcriptGuilds: [...transcriptGuilds], sectionGrants: [...sectionGrants] };
    const at = Date.now();
    grantsCache.set(discordId, { v, at, ver });
    kvSetJSON(`sess:grants:${discordId}`, { v, at, ver }, LEVEL_TTL_MS / 1000);
  } catch { /* keep last known */ }
  return v;
}
...
export const getSession = cache(async () => {
  ...
  // Global revocation: a token minted before the kill cutoff is dead, forcing a fresh login. No cutoff
  // set (the default) → this is a no-op. Applies to everyone, including super owners — the one token you
  // most want to be able to kill — since re-login is a single Discord click.
  const cutoff = await killBefore();
  if (cutoff && Number(payload.iat) && payload.iat < cutoff) return null;
  ...
  const [scope, { manualPerms, featureGrants, securityGuildIds, sharedGuildIds, group, transcriptGuilds, sectionGrants }] = await Promise.all([
  ...
  // A guild is reachable via admin/owner, a manual perm, a direct feature grant, antinuke-admin, OR a
  // Role-Access "View transcripts" grant (transcriptGuilds) — otherwise a transcript-only grant would
  // resolve but leave the user with no reachable server and get them denied at the door.
  const serverGuildIds = [...new Set([...adminGuildIds, ...Object.keys(manualPerms), ...Object.keys(featureGrants || {}), ...securityGuildIds, ...(transcriptGuilds || []), ...(owner ? (sharedGuildIds || []) : [])])];
  // Game/whitelist section: Roblox staff ladder, scoped group, delegated Role-Access group access, OR a
  // delegated section grant (bans / powers / grants).
  const sectionGrantList = Array.isArray(sectionGrants) ? sectionGrants : [];
  const gameAccess = owner || level > 0 || scopedGroup || !!roleGroup || sectionGrantList.length > 0;
  ...
  return { ...payload, level: eff, role: labelForLevel(eff), isOwner: owner, scopedGroup, scope, serverPerms, manualPerms, featureGrants: featureGrants || {}, securityGuildIds, serverGuildIds, group: roleGroup, transcriptGuilds: transcriptGuilds || [], sectionGrants: sectionGrantList, gameAccess };
});
