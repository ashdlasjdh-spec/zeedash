...
// The Game/whitelist section is gated by the ladder above. The Server section is gated by the
// signed-in user's Discord permissions in each guild (session.serverPerms), with Roblox staff kept
// as an override so the product owners still manage every server.
// The most destructive security features are locked tighter: only the guild owner, an antinuke
// admin, or top staff — never a plain Discord admin.
...
// Roblox staff overrides: head-of-staff+ (canGroup) may manage any server; co-owners+ (canConfig)
// also override the security lock.
export function staffCanManageServers(level) { return canGroup(level); }
export function staffCanManageSecurity(level) { return canConfig(level); }
// Does this session hold Discord admin/owner in the given guild?
...
// Manage a guild's non-security server settings.
export function canManageGuild(session, guildId) {
  if (!session || !guildId) return false;
  return staffCanManageServers(session.level) || guildAdminOf(session, guildId);
}
// Can the session reach the Server section at all (drives nav visibility)?
export function canAccessServerSection(session) {
  if (!session) return false;
  return staffCanManageServers(session.level) || (session.serverGuildIds?.length || 0) > 0;
}
