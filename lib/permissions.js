...
// The group ranks a scoped user may assign / kick, matched by role name.
export const SCOPED_RANK_RE = /crew\s*lead|leaderboard/i;
...
// Moderation (all ban/unban/kick/warn + bulk) is co founders+ (254) only.
export function canBan(level)       { return (Number(level) || 0) >= LVL.TAGEMOJI; }
export function canBulkBan(level)   { return (Number(level) || 0) >= LVL.TAGEMOJI; }
