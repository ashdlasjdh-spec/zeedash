...
// signed-in user's Discord permissions in EACH guild (session.serverPerms) — the Roblox rank ladder
// grants NO access here. A director in the main server with no Discord perms in another server does
// not see that other server at all. The most destructive security features are locked tighter still:
// only the guild owner or an antinuke admin — never a plain Discord admin.
...
// Can the session reach the Server section at all (drives nav visibility)? Only if they hold
// Discord admin/owner in at least one server the bot shares with them.
