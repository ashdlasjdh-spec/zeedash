...
// Baked-in Discord role -> level, so a known role keeps working even if it isn't
// in DISCORD_ROLE_MAP yet. Applied as a FLOOR (max with the env map), so a role
// here can never resolve BELOW its baked level. Extend via ROLE_LEVEL_OVERRIDES
// (JSON "<roleId>": <level>). 254 = co founders (full site access).
const ROLE_LEVEL_OVERRIDES = { "1451418255385301033": 254 };
function roleOverrides() {
  try { return { ...ROLE_LEVEL_OVERRIDES, ...JSON.parse(process.env.ROLE_LEVEL_OVERRIDES || "{}") }; }
  catch { return { ...ROLE_LEVEL_OVERRIDES }; }
}

// discordRoleId -> level. Highest wins. The DISCORD_ROLE_MAP env is merged with the
// baked-in overrides above, taking whichever level is higher per role.
export function roleMap() {
  let env = {};
  try { env = JSON.parse(process.env.DISCORD_ROLE_MAP || "{}"); } catch { env = {}; }
  const out = { ...env };
  for (const [id, lvl] of Object.entries(roleOverrides())) {
    const e = Number(out[id]);
    const o = Number(lvl);
    if (!Number.isFinite(o)) continue;
    out[id] = Number.isFinite(e) ? Math.max(e, o) : o;
  }
  return out;
}
