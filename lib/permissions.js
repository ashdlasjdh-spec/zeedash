...
export function canManageFeature(session, guildId, feature) {
  ...
  // Fake permissions mints who the bot treats as admin, so it's the tightest non-security gate: the
  // real server OWNER (or a super owner) only — never a plain Discord admin, never a manual perm.
  if (String(feature) === "fake-permissions") return guildOwnerOf(session, guildId);
