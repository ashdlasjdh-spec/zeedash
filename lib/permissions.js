...
export const FEATURE_PERM = {
  ...
  levels: "manage_guild", pingonjoin: "manage_guild", tracking: "manage_guild",
  autopfp: "manage_guild",
  // scheduled / event features -> Manage events
  timers: "manage_events",
...
// fake-permissions config's per-role `features`. Shape: { slug: { access:"view"|"manage", channels:[] } }.
export function featureGrantsOf(session, guildId) {
  const v = session?.featureGrants?.[guildId];
  return v && typeof v === "object" ? v : {};
}
export function featureGrantOf(session, guildId, feature) {
  return featureGrantsOf(session, guildId)[String(feature || "")] || null;
}
export function hasFeatureGrant(session, guildId, feature) {
  return !!featureGrantOf(session, guildId, feature);
}
// The slugs a role may VIEW (any grant) — used for read-only reach; the manage subset is via canManageFeature.
export function featurePermsOf(session, guildId) {
  return Object.keys(featureGrantsOf(session, guildId));
}
// Channels a grant is limited to for a feature (empty array = all channels). Drives the read-only
// channel-picker restriction on the feature pages; enforcement of the actual write is UI + bot-side.
export function grantChannelsFor(session, guildId, feature) {
  const g = featureGrantOf(session, guildId, feature);
  return g && Array.isArray(g.channels) ? g.channels : [];
}
...
export function canManageFeature(session, guildId, feature) {
  ...
  // A MANAGE-level direct grant, OR a manual Discord-permission that unlocks the feature. A view-only
  // grant does NOT satisfy manage (see canViewFeature).
  const g = featureGrantOf(session, guildId, feature);
  if (g && g.access !== "view") return true;
  return hasManualPerm(session, guildId, featurePerm(feature));
}
// Can the session at least SEE this feature's config (manage, or a read-only view grant)?
export function canViewFeature(session, guildId, feature) {
  if (canManageFeature(session, guildId, feature)) return true;
  return hasFeatureGrant(session, guildId, feature); // a view-only grant
}
// True when the session may see the feature but only read-only (a view grant, no manage path).
export function isFeatureReadOnly(session, guildId, feature) {
  return canViewFeature(session, guildId, feature) && !canManageFeature(session, guildId, feature);
}
// The non-security feature slugs the session may MANAGE in a guild — every one for a Discord admin,
// or the subset a manual-permission holder's perms + manage-grants unlock. Drives nav + overview
...
// The non-security feature slugs the session may VIEW (manage OR read-only). Drives nav visibility so a
// view-only grant still shows the page.
export function viewableFeatures(session, guildId) {
  return NONSECURITY_FEATURES.filter((f) => canViewFeature(session, guildId, f));
}
...
export const GROUP_ACTIONS = [
  "lookup", "rank", "promote", "demote", "accept", "decline", "kick", "shout", "acceptAll", "declineAll",
  // Scoped accept/kick queues — a role can be limited to just one staff track (e.g. only Crew, only
  // Leaderboard). The plain accept/kick grants still cover every track; these narrow it to one.
  "lbAccept", "crewAccept", "starAccept", "lbKick", "crewKick", "starKick",
  "contentAccept", "contentStaffAccept", "contentKick", "contentStaffKick",
];
export const GROUP_ACTION_LABELS = {
  ...
  lbAccept: "Accept + rank Leaderboard Staff", crewAccept: "Accept + rank Crew", starAccept: "Accept + rank Stars",
  lbKick: "Kick Leaderboard Staff", crewKick: "Kick Crew", starKick: "Kick Stars",
  contentAccept: "Accept + rank Content", contentStaffAccept: "Accept + rank Content Staff",
  contentKick: "Kick Content", contentStaffKick: "Kick Content Staff",
};
// Actions where the person PICKS a variable target rank — only these are bounded by the maxRank ceiling.
// The scoped accepts (lbAccept/crewAccept/starAccept) assign a FIXED rank (the track's own), so they
// don't need — and shouldn't require — a ceiling.
export const RANK_ASSIGN_ACTIONS = new Set(["rank", "promote", "accept", "acceptAll"]);

// Dashboard SECTION grants delegated via Role Access (on top of, or instead of, the numeric level).
// These open whole game-management areas to a role or a specific user: in-game bans, powers granting,
// and all granting. Resolved live into session.sectionGrants; the same helpers below gate the nav AND
// the API routes, so a grant is enforced end-to-end.
export const SECTION_GRANTS = ["bans", "powers", "grants"];
export const SECTION_GRANT_LABELS = {
  bans: "Bans & moderation",
  powers: "Powers granting",
  grants: "All granting (powers, stands, tools, gamepasses, tags, emojis…)",
};
export function sectionGrantsOf(session) {
  return Array.isArray(session?.sectionGrants) ? session.sectionGrants.filter((s) => SECTION_GRANTS.includes(s)) : [];
}
...
// Ticket-transcript viewing, granted via Role Access ("View transcripts" per role). A super owner may
// view any; otherwise the session must hold a mapped role granting it in that guild. The per-ticket
// "roles that could see the channel" check (allowedRoles on the transcript) is applied at view time.
export function transcriptGuildsOf(session) {
  return Array.isArray(session?.transcriptGuilds) ? session.transcriptGuilds : [];
}
export function canViewTranscripts(session, guildId) {
  if (!session) return false;
  if (session.isOwner) return true;
  return transcriptGuildsOf(session).includes(String(guildId || ""));
}

// Session-aware gates: pass the level check OR the delegated access. Delegation can come from the
// numeric level, Role-Access group access, OR Role-Access SECTION grants (bans / powers / grants).
// These same helpers gate both the nav and the API routes, so a grant is enforced end-to-end.
export function canCat(session, category) {
  const secs = sectionGrantsOf(session);
  if (secs.includes("grants")) return GRANT_CATS.includes(category);   // full granting
  if (secs.includes("powers") && category === "power") return true;    // powers-only grant
  return can(session?.level, category);
}
export function canBanS(session) { return canBan(session?.level) || sectionGrantsOf(session).includes("bans"); }
export function canWhitelistS(session) { return canWhitelist(session?.level); }
export function canManageGrantsS(session) { return canManageGrants(session?.level) || sectionGrantsOf(session).includes("grants"); }
...
// Does this session have ANY game access at all (ladder level, scoped group, delegated group, or a
// delegated section grant)?
export function hasGameAccess(session) {
  return !!(session?.isOwner || (Number(session?.level) || 0) > 0 || session?.scopedGroup || groupAccessOf(session) || sectionGrantsOf(session).length);
}
