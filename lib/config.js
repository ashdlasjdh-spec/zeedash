...
export async function getConfig() {
  ...
  return {
    ...
    // Ban-log destination. Preferred: post AS THE BOT to a channel — needs a bot token + channel id.
    // Falls back to a Discord webhook. All kept in the DB (private), never committed (public repos).
    botToken: clean(o.bot_token || process.env.DISCORD_BOT_TOKEN),
    botTokenSource: o.bot_token ? "dashboard" : (process.env.DISCORD_BOT_TOKEN ? "env" : "none"),
    banLogChannel: clean(o.ban_log_channel || process.env.BAN_LOG_CHANNEL_ID) || "1536813165189537844",
    banWebhook: clean(o.ban_webhook || process.env.BAN_WEBHOOK_URL),
    banWebhookSource: o.ban_webhook ? "dashboard" : (process.env.BAN_WEBHOOK_URL ? "env" : "none"),
