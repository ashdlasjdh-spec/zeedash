...
export async function getBanTrends(days = 14) {
  ...
  const bans14 = series.reduce((s, r) => s + r.bans, 0);
  // Observability: if the endpoint responded (ok) but we parsed 0 events, the log shape has likely
  // drifted — this line in the server logs makes that obvious instead of a silently-empty graph.
  console.log(`[bantrends] ok=${ok} scanned=${total} bans(${days}d)=${bans14}` + (ok && total === 0 ? " — endpoint returned no parseable logs (shape drift?)" : ""));
  const data = { series, scannedAt: new Date().toISOString(), ok, scanned: total };
