...
// Guilds ALWAYS checked for permission roles, on top of DISCORD_GUILD_ID and the
// fallback below. Baked in so a known permission server keeps working without env
// changes; extend via DISCORD_EXTRA_GUILD_IDS (CSV). The bot must be in each of
// these guilds with Server Members Intent for its roles to be read.
const EXTRA_PERMISSION_GUILD_IDS = ["1447037325380157452"];

// A member's level = the HIGHEST mapped level across the permission guilds — the main server
// (DISCORD_GUILD_ID), the staff server (DISCORD_FALLBACK_GUILD_ID, default the staff/training
// guild), and any EXTRA_PERMISSION_GUILD_IDS. So a staff role in ANY of them grants dashboard
// access, matching the bot. Roles are matched by ID via DISCORD_ROLE_MAP, so only mapped roles count.
export async function levelFromDiscord(userId) {
  const extra = (process.env.DISCORD_EXTRA_GUILD_IDS || "").split(",").map((s) => s.trim()).filter(Boolean);
  const guildIds = [...new Set([
    process.env.DISCORD_GUILD_ID,
    process.env.DISCORD_FALLBACK_GUILD_ID || "1531917648588312677",
    ...EXTRA_PERMISSION_GUILD_IDS,
    ...extra,
  ].filter(Boolean))];
  let level = 0;
