...
// Cached ~60s per guild (channels/roles change rarely) so repeat page loads are instant.
...
export async function getGuildMeta(guildId) {
  ...
  const pick = (types) => chans.filter((c) => types.includes(c.type)).sort(byPos).map((c) => ({ id: c.id, name: c.name }));
  const data = {
