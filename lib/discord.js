...
// Update one AutoMod rule's keyword filter (KEYWORD rules only).
export async function updateAutomodKeywords(guildId, ruleId, keywords) {
  ...
  const res = await fetch(`${API}/guilds/${guildId}/auto-moderation/rules/${ruleId}`, {
    ...
    body: JSON.stringify({ trigger_metadata: { keyword_filter: keywords.slice(0, 1000) } }),
  });
  if (!res.ok) return { error: `Discord returned ${res.status}`, status: res.status };
  return { ok: true };
}
