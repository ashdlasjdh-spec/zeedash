...
export async function postChannelMessage(channelId, { content, embed, profile } = {}) {
  ...
  try {
    ...
    if (res.ok) { const j = await res.json().catch(() => ({})); return { ok: true, id: j?.id ? String(j.id) : null }; }
    const j = await res.json().catch(() => ({}));
...
// Edit an existing bot-posted message in place (the "live embed" update). content/embed replace the
// message body. Only works for messages the BOT itself sent (Discord won't let a bot edit others').
export async function editChannelMessage(channelId, messageId, { content, embed } = {}) {
  const token = process.env.DISCORD_BOT_TOKEN;
  if (!token) return { ok: false, error: "The dashboard has no bot token configured (DISCORD_BOT_TOKEN)." };
  if (!channelId || !messageId) return { ok: false, error: "No message to update." };
  const body = { content: content ? String(content).slice(0, 2000) : "", embeds: embed ? [embed] : [] };
  if (!body.content && !body.embeds.length) return { ok: false, error: "Nothing to show — add text or an embed." };
  try {
    ...
    if (res.ok) return { ok: true };
    const j = await res.json().catch(() => ({}));
    if (res.status === 404) return { ok: false, error: "That message is gone — it was deleted in Discord. Post it again." };
    if (res.status === 403) return { ok: false, error: "The bot can't edit that message (only messages it posted itself can be edited)." };
    return { ok: false, error: j?.message || `Discord returned ${res.status}.` };
  } catch (e) {
    return { ok: false, error: e.message };
  }
...
export async function deleteChannelMessage(channelId, messageId) {
  const token = process.env.DISCORD_BOT_TOKEN;
  if (!token || !channelId || !messageId) return { ok: false, error: "Nothing to delete." };
  try {
    const res = await fetch(`${API}/channels/${channelId}/messages/${messageId}`, {
      method: "DELETE", headers: { Authorization: `Bot ${token}` },
    });
    if (res.ok || res.status === 404) return { ok: true }; // already gone = fine
