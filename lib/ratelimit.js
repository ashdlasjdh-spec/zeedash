...
// Route-handler convenience: enforce a limit for `key` and return a ready-to-send 429 when exceeded,
// or null to proceed. Fails open (returns null) when Upstash isn't configured or errors. Use after the
// route's own auth check, keyed by user id, to cap the most destructive actions per-USER — which the
// per-IP middleware limit can't do against an attacker rotating IPs.
export async function limited(key, opts) {
