...
// A role mapping is expired when its `expires` (epoch ms) is in the past. No expiry = permanent.
export function isExpired(item, now = Date.now()) {
  const e = Number(item?.expires);
  return Number.isFinite(e) && e > 0 && e <= now;
}

// Normalize a whole items array to the canonical shape, dropping junk & duplicate roles.
// Set `dropExpired` to also drop rows whose expiry has passed (used server-side to prune on save).
export function normalizeItems(items, { dropExpired = false } = {}) {
  const out = [];
  const seen = new Set();
  for (const it of Array.isArray(items) ? items : []) {
    const role = String(it?.role || "").match(/^\d{5,}$/)?.[0];
    if (!role || seen.has(role)) continue;
    if (dropExpired && isExpired(it)) continue;
    ...
    seen.add(role);
    const row = { role, perms: perms.join(","), features: feats };
    const exp = Number(it?.expires);
    if (Number.isFinite(exp) && exp > 0) row.expires = exp;
    out.push(row);
  }
  return out;
}
...
export const PRESETS = [
  ...
  { key: "event_host", label: "Event Host", perms: ["manage_events"], features: ["timers"] },
...
export function resolveFakeForRoles(config, roleIds) {
  ...
  for (const it of normalizeItems(config?.items)) {
    if (!ids.has(String(it.role)) || isExpired(it)) continue;
