...
// Post a ban-log message to the webhook. With files it goes as multipart (embed + attachments in one
// message, like the moderation apps) and falls back to embed-only if that's rejected (e.g. a file over
// the server's upload limit). Shared by the bot path (banAction) and the site path (app/api/bans/route).
// `files` is [{ name, blob }]. Returns a short status string. Pure (globals only) so it's unit-testable.
export async function sendBanWebhook(hook, { embed, contentUrl, files }) {
  const embedOnly = () =>
    fetch(hook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ embeds: [embed], allowed_mentions: { parse: [] }, ...(contentUrl ? { content: contentUrl } : {}) }) });
  ...
  try {
    ...
    if (prepared.length) {
      ...
      const payload = {
        ...
        attachments: prepared.map((f, i) => ({ id: i, filename: f.name })),
        ...(contentUrl ? { content: contentUrl } : {}),
      };
      form.append("payload_json", JSON.stringify(payload));
      prepared.forEach((f, i) => form.append(`files[${i}]`, f.blob, f.name));
      wr = await fetch(hook, { method: "POST", body: form });
      if (!wr.ok) wr = await embedOnly(); // file too big for the log server → still post the embed
    } else {
      wr = await embedOnly();
    }
    return wr.ok ? "sent" : `webhook ${wr.status}`;
  } catch (e) {
