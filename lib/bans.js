...
export async function banAction({ input, reason, duration, evidence, action = "ban", actorName, actorId }) {
  ...
  if (hook) {
    ...
    try {
      const wr = await fetch(hook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ embeds: [embed], allowed_mentions: { parse: [] }, ...(ev.contentUrl ? { content: ev.contentUrl } : {}) }) });
