  ...
  if (hook) {
    ...
    try {
      ...
      if (prepared.length) {
        ...
        wr = await fetch(hook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ embeds: [embed], allowed_mentions: { parse: [] }, ...(ev.contentUrl ? { content: ev.contentUrl } : {}) }) });
      }
