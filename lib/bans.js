...
import { evidenceParts, selectBanFiles, MAX_BAN_FILE_BYTES, MAX_BAN_TOTAL_BYTES } from "@/lib/banEvidence";
...
export async function banAction({ input, reason, duration, evidence, note, files, action = "ban", actorName, actorId }) {
  ...
  if (hook) {
    ...
    const embedOnly = () =>
      fetch(hook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ embeds: [embed], allowed_mentions: { parse: [] }, ...(ev.contentUrl ? { content: ev.contentUrl } : {}) }) });
    ...
    try {
      ...
      if (prepared.length) {
        ...
        const payload = {
          ...
          ...(ev.contentUrl ? { content: ev.contentUrl } : {}),
        };
        form.append("payload_json", JSON.stringify(payload));
        prepared.forEach((f, i) => form.append(`files[${i}]`, f.blob, f.name));
        wr = await fetch(hook, { method: "POST", body: form });
        // A file over the log server's upload limit 413s the whole message — fall back to embed-only so
        // the log entry (reason, case id, links) still posts.
        if (!wr.ok) wr = await embedOnly();
      } else {
        wr = await embedOnly();
      }
