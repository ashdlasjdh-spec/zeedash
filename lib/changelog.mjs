...
function isInternal(subject) {
  ...
  // Never surface any commit that mentions the AI tooling on the public feed.
  if (/\b(claude|anthropic|co-authored|copilot)\b/.test(t)) return true;
...
export async function getChangelog(perRepo = 25) {
  ...
  await Promise.all(
    Object.keys(REPOS).map(async (repo) => {
      try {
        // A GITHUB_TOKEN (any classic/fine-grained token with public-repo read) lifts the API limit from
        // 60/hr to 5000/hr — optional, but makes the page immune to shared-IP rate-limiting. Works without.
        const headers = { Accept: "application/vnd.github+json", "User-Agent": "zhd-changelog" };
        ...
        for (const c of Array.isArray(arr) ? arr : []) {
          ...
          entries.push({
            ...
            // No commit URL: the repos are private, so a github.com link would 404 / leak the repo.
          });
        }
