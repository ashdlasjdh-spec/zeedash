...
import {
  ...
  hasManualPerm, featurePerm, manageableFeatures, viewableFeatures,
} from "./permissions";
...
export async function canEditFeature(session, guildId, feature) {
  if (!session || !guildId) return false;
  // Security features + fake-permissions gate on security standing (owner / super / antinuke admin).
  if (isSecurityFeature(feature) || String(feature) === "fake-permissions") return canManageSecurity(session, guildId);
...
export async function guildAccess(session, guildId) {
  ...
  return { manage, security, owner: guildOwnerOf(session, guildId), manageable: manageableFeatures(session, guildId), viewable: viewableFeatures(session, guildId) };
}
