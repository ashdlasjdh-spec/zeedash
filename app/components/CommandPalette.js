...
export default function CommandPalette({ items = [] }) {
  ...
  const results = useMemo(() => {
    ...
    const list = !s ? items : items.filter((it) => it.label.toLowerCase().includes(s) || it.group.toLowerCase().includes(s));
    return list.slice(0, 40);
  }, [q, items]);
  useEffect(() => { setIdx(0); }, [q]);

  function go(it) { if (!it) return; setOpen(false); router.push(it.href); }
    ...
    <div className="cmdk-backdrop" onMouseDown={() => setOpen(false)}>
      <div className="cmdk" onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="cmdk-list">
          ...
          {results.map((it, i) => (
            <button key={it.href} className={`cmdk-item ${i === idx ? "sel" : ""}`} onMouseEnter={() => setIdx(i)} onClick={() => go(it)}>
