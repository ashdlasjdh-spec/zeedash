// Shared, server-rendered building blocks for the zhd.lol docs.
// No client JS — every doc page is a static server component, so the docs load
// instantly and pass the strict CSP with zero inline scripts.
import Link from "next/link";

const P = { fill: "none", stroke: "currentColor", strokeWidth: 1.7, strokeLinecap: "round", strokeLinejoin: "round" };

// Line-icon set (matches the dashboard sidebar icons).
export const ICONS = {
  ...
  key: <><circle cx="8" cy="8" r="4" {...P} /><path d="m11 11 8 8M16 16l2-2M19 19l2-2" {...P} /></>,
  robot: <><rect x="5" y="7" width="14" height="11" rx="3" {...P} /><path d="M12 3v4M8.5 12h.01M15.5 12h.01M9 16h6" {...P} /></>,
  spark: <path d="M12 3v6M12 15v6M3 12h6M15 12h6M6 6l3.5 3.5M14.5 14.5 18 18M18 6l-3.5 3.5M9.5 14.5 6 18" {...P} />,
  info: <><circle cx="12" cy="12" r="9" {...P} /><path d="M12 11v5M12 7.5h.01" {...P} /></>,
  chart: <><path d="M4 20V4M4 20h16" {...P} /><path d="M8 16v-4M12 16V8M16 16v-6" {...P} /></>,
};

export function Icon({ name, size = 20 }) {
  return <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true">{ICONS[name] || null}</svg>;
}

// A framed "screenshot" of any children — browser chrome + soft glow, so mock UIs
// and diagrams read as real product images.
export function Figure({ url = "zhd.lol", children, caption }) {
  return (
    <figure className="figure">
      <div className="figure-frame">
        <div className="figure-chrome">
          <span className="dot" /><span className="dot" /><span className="dot" />
          <span className="fc-url">{url}</span>
        </div>
...
export function Callout({ kind = "info", children }) {
  const ic = kind === "warn" ? "info" : kind === "good" ? "shield" : "info";
  return (
    <div className={`callout ${kind}`}>
      <span className="co-ic"><Icon name={ic} size={18} /></span>
      <div>{children}</div>
    </div>
  );
}
...
export function Tiles({ items }) {
  return (
    <div className="doc-grid">
      {items.map((t) => (
        <div className="doc-tile" key={t.title}>
          <div className="dt-ic"><Icon name={t.icon} size={19} /></div>
          <h4>{t.title}</h4>
          <p>{t.body}</p>
        </div>
      ))}
...
export function Steps({ items }) {
  return (
    <ol className="steps">
      {items.map((s) => (
        <li key={s.title}><h4>{s.title}</h4><p>{s.body}</p></li>
      ))}
    </ol>
  );
}
...
export function Pager({ prev, next }) {
  return (
    <nav className="doc-pager">
      {prev ? <Link href={prev.href}><span className="dp-k">← Previous</span><span className="dp-t">{prev.title}</span></Link> : <span style={{ flex: 1 }} />}
      {next ? <Link className="next" href={next.href}><span className="dp-k">Next →</span><span className="dp-t">{next.title}</span></Link> : <span style={{ flex: 1 }} />}
    </nav>
  );
}
